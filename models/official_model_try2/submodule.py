...
class disparityregression_expend(nn.Module):
    def __init__(self, maxdisp):
        super().__init__()
        self.maxdisp = maxdisp
        self.expend_maxdisp = maxdisp+10
        self.disp = torch.Tensor(np.reshape(np.array(range(self.expend_maxdisp))-5,[1, self.expend_maxdisp, 1, 1])).cuda()

    def forward(self, x):
        size = x.shape
        expend_x = torch.zeros([size[0], self.expend_maxdisp, size[2], size[3]]).contiguous().cuda()
        expend_x[:, 5:5+192, :,:] = x
        for i in range(6):
            expend_x[:, i, :, :] = x[:, 5-i, :, :]
            expend_x[:, 5+191+i, :, :] = x[:, -i-1, :, :]
        out = torch.sum(expend_x*self.disp.data,1, keepdim=True)
        return out
