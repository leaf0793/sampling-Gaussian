...
class PSMNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        if self.training is False:
            # pred3 = disparityregression_expend(self.maxdisp)(pred3)
...
class PSMNet_KD(PSMNet):
    def __init__(self, maxdisp):
        super().__init__(maxdisp)

    def forward(self, left, right, temparture=1.0):
        KL = []
        refimg_fea     = self.feature_extraction(left)
        targetimg_fea  = self.feature_extraction(right)
        ...
        cost1 = F.interpolate(cost1, [self.maxdisp,left.size()[2],left.size()[3]], mode='trilinear')
        cost1 = torch.squeeze(cost1,1)
        pred1 = F.softmax(cost1/temparture, dim=1)
        KL.append(pred1)
        
        cost2 = F.interpolate(cost2, [self.maxdisp,left.size()[2],left.size()[3]], mode='trilinear')
        cost2 = torch.squeeze(cost2,1)
        pred2 = F.softmax(cost2/temparture, dim=1)
        KL.append(pred2) 

        cost3 = F.interpolate(cost3, [self.maxdisp,left.size()[2],left.size()[3]], mode='trilinear')
        cost3 = torch.squeeze(cost3,1)
        pred3 = F.softmax(cost3/temparture, dim=1)
        KL.append(pred3)
        if self.training:
            pred1 = disparityregression(self.maxdisp)(pred1)
            pred2 = disparityregression(self.maxdisp)(pred2)
